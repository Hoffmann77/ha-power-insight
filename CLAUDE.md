...
Engine-tier tests in `tests/engine/` import `power_insight.py` directly via `importlib.util` (in `tests/engine/home.py`) to bypass all HA dependencies. The hand-derived harnesses and the reference cases use **declarative homes** (`tests/engine/home.py`); `test_flow_view.py` uses the **source-order scenario framework** (`tests/engine/scenario_framework.py`, wired via `conftest.py`). Both build on the plain data in `home.py`: `Adapter` / `Topology` / `State` / `Cell`, which the generated homes and frozen snapshots also use directly.
