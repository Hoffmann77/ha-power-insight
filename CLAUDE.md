...
**Engine edge-case framework** (`tests/engine_property_framework.py`): a declarative environment for pinning down individual engine-property return values in edge cases. An `EngineCase` pairs a device configuration (a `PRESET_DEVICES` name or a custom `DeviceConfig` built from `GridSpec`/`PvSpec`/`BatterySpec`/`ConsumerSpec`) and a set of entity readings (a `PRESET_ENTITIES` name or a raw `{entity_id: watts}` map, plus optional `entity_overrides`) with an `expected` block mapping property names to hand-written expected values. Matching handles numbers (tolerant), `None`, exact dicts, `subset(...)` partial dicts, and predicate callables; `assert_engine_case()` aggregates every mismatch into one message. Unlike `test_power_insight_calculations.py` (which re-derives expectations from the engine's own formulas), here you write the expected number out by hand.

Two authoring styles sit on this shared layer: a **declarative** list of `EngineCase` objects in `tests/test_engine_property_cases.py` (add a case by appending to `CASES`, no new test method needed), and a **class-per-scenario** style in `tests/test_engine_property_scenarios.py` that mirrors `TestFullScenario` — each class pins exactly one `DEVICE` + one `ENTITIES` reading set and each `test_` method asserts one property, using the `build_engine_for(device, entities, overrides=None)` helper in a fixture. A scenario class never sweeps several entity-value sets; test a different reading set by writing another class. Both reuse the same `DeviceConfig`/preset/validation machinery.
