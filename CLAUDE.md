...
Two authoring styles sit on this shared layer: a **declarative** list of `EngineCase` objects in `tests/test_engine_property_cases.py` (add a case by appending to `CASES`, no new test method needed), and a **class-per-scenario** style in `tests/test_engine_property_scenarios.py` that mirrors `TestFullScenario` — a class names a `DEVICE` + entity readings and each `test_` method asserts one property, using the `build_engine_for(device, entities, overrides=None)` helper in a fixture. Both reuse the same `DeviceConfig`/preset/validation machinery.
