...
**Reference corpus** (`tests/engine/reference/`): nine small homes — one module per case, each a `Case` holding a wiring, its `Snapshot`s (readings + price), and an `answers` dict of values *derived by hand from the model*, never read back from the engine. `test_reference_corpus.py` generates one test per written answer; an answer of `None` asserts the engine publishes nothing at all, and a property left out of the dict generates no test. Pass/fail only — a red test means either the engine or the derivation is wrong, and resolving which is a human call. `tools/export_cases.py` projects the same cases into `docs/spec/cases/*.json` for the documentation site (`--check` in CI fails when that output is stale); the JSON is generated and must never be hand-edited.
