...
**Engine edge-case framework** (`tests/engine_property_framework.py`): shared infrastructure for pinning down individual engine-property return values in edge cases. It provides declarative adapter specs (`GridSpec`/`PvSpec`/`BatterySpec`/`ConsumerSpec`), a `DeviceConfig` that assembles them, a set of `PRESET_DEVICES` (`grid_only`/`grid_pv`/`grid_pv_battery`/`full`), and `build_engine_for(device, entities)` — which takes a preset device name or a `DeviceConfig` plus a raw `{entity_id: watts}` mapping (engine sign convention), validates every id against the device's routable entities (raising `ValueError` on a typo/mismatch), and returns a ready-to-query `PowerInsight`.

Tests using it live in `tests/test_engine_property_scenarios.py` in a **class-per-scenario** style that mirrors `TestFullScenario`: each class pins exactly one `DEVICE` + one `ENTITIES` reading set, a `pi` fixture calls `build_engine_for`, and each `test_` method asserts one property against a hand-written expected value (`pytest.approx` for floats). Unlike `test_power_insight_calculations.py` (which re-derives expectations from the engine's own formulas), here you write the expected number out by hand. A scenario class never sweeps several entity-value sets — to test a different reading set, add another class.
