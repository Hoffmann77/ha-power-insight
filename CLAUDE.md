...
Adapters are declared with the `Adapter.grid()/pv()/battery()/consumer()` factories, config inline at the call site (`Adapter.pv("pv1", lcoe=0.10, exports=True)`, `Adapter.battery("bat1", charge_from=("pv1",))`, `Adapter.consumer("plug", power_from=("pv1",))`). A `@state` is `State(grid=1000, pv1=-500, price=0.30)`; `None` power models an unavailable sensor. A safety rail requires a state to name *exactly* its topology's uids. Each test receives a freshly built engine via the `power_insight` fixture (or `state` / `topology` for the raw block objects), and asserts hand-written expected values derived from first principles — not read back from the engine — so a regression flips the test red. Hand-derived values are asserted in exactly two places. `tests/engine/manual/` holds one explicit harness per **engine decision**: one module per area (`test_power_flow.py` → `TestPowerFlow`), one block per decision — the smallest `@topology` + `@state` that tells the decision apart from its alternatives, then `@expect_attribute` claims derived by hand; the `@state` docstring names the decision and its note in `docs/dev/engine-calculations.md`. Whenever an engine decision is made (or a bug turns into one), add its block there alongside the note and the fix. `tests/engine/reference/` is the documentation corpus (below). The files beside them survive only because no fixed case could express them: `test_flow_view.py` (grouping properties return adapter objects, share properties return a `(vector, index)` pair — no catalogable shape, no sensor renders them), `test_source_shares_invariants.py` (holds over generated topologies; a fixed case cannot say "for all"), `test_snapshot_cache.py` (memo invalidation across a reading change — a question about time), `test_scenario_framework.py` (self-tests for the machinery the corpus runs on). Two value-free files prototype a replacement strategy: `test_identities.py` (one executable formula per catalogued property over its published dependencies, checked on random homes from `random_homes.py`; provenance is the only root) and `test_laws.py` (metamorphic laws — power/price scaling by catalog unit, renaming, idle devices, PV-system splitting, unavailability, conservation). Anything else that asserts a hand-derived value belongs in `manual/` or `reference/`. Note the live consequence: the catalog covers 50 of the 64 sensor-facing properties, and no case yet wires a grid-anchored `charge_from=("grid", …)` restriction, so the remainder is uncovered until the corpus catches up — see `tests/README.md`.

**Reference corpus** (`tests/engine/reference/`): ten small homes — one module per case, each an ordinary `EngineScenario` subclassing `ReferenceCase` with `case_id` / `title`. Its `@expect("<property>")` methods take only `self` and return a value *derived by hand from the model*, never read back from the engine; `@expect` is `expect_attribute` with the tolerance for that property's unit from `docs/spec/properties.json`, so a misspelled name raises at import. Returning `None` asserts the engine publishes nothing at all. Every snapshot already declares a method for all 56 catalogued properties in dependency order, most still `return TODO` — a `TODO` stub skips, publishes nothing, and turns into a real assertion the moment somebody replaces it with a value; the skip message carries that property's definition and steps, so `pytest <case> -rs` reads as a worklist. `test_corpus.py` enforces that the scaffold stays complete. Pass/fail only — a red test means either the engine or the derivation is wrong, and resolving which is a human call.

The same classes are the documentation. `ReferenceCase.publish()` reads a case back out via `scenario_blocks()` — the framework's source-order binding applied outside pytest, so the published snapshot is by construction the asserted one. Prose comes from docstrings: the class docstring above its `Decides:` list is the page summary, a `@state` docstring is its snapshot caption, and an `Open question:` paragraph inside one becomes a docs callout. `tools/export_cases.py` writes `docs/spec/cases/*.json` (generated — never hand-edit); `reference/test_corpus.py` fails when that output is stale.
