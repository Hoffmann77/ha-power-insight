...
Engine-tier tests in `tests/engine/` import `power_insight.py` directly via `importlib.util` to bypass all HA dependencies, and use the **source-order scenario framework** (`tests/engine/scenario_framework.py`, wired via `conftest.py`).
...
The engine tier asks three questions, one directory each:
...
- **`tests/engine/manual/`** — one hand-derived harness per **engine decision**: one module per area (`test_power_flow.py` → `TestPowerFlow`, `test_edge_readings.py` → `TestEdgeReadings`), one block per decision — the smallest `@topology` + `@state` that tells the decision apart from its alternatives, then `@expect_attribute` claims derived by hand, never read back from the engine. The `@state` docstring names the decision and its note in `docs/dev/engine-calculations.md`. Whenever an engine decision is made (or a bug turns into one), add its block alongside the note and the fix.
- **`tests/engine/reference/`** — the docs showcase: ten fixed homes (`ReferenceCase` subclasses with `case_id` / `title`, one `@topology`, a few `@state`s, prose in docstrings) that assert nothing. `tools/export_cases.py` passes every snapshot through the engine and writes every catalogued property to `docs/spec/cases/*.json` (generated — never hand-edit). The JSON is committed and `reference/test_corpus.py` fails when it no longer matches the engine, so re-export in the same commit as any engine change that moves a published number; a docs version cut from any commit freezes that commit's own results. Prose: the class docstring above its `Shows:` list is the page summary, a `@state` docstring is its snapshot caption, and an `Open question:` paragraph becomes a callout. The docs component in `website/src/components/CaseDiagram/` is shared by every docs version, so it must keep reading older versions' case JSON (`resultsOf` accepts the legacy `expectations` field).

Beside them: `test_flow_view.py` (grouping properties return adapter objects and `(vector, index)` pairs — no catalogable shape), `test_snapshot_cache.py` (memo invalidation across a reading change — a question about time), `test_scenario_framework.py` (self-tests for the framework).
